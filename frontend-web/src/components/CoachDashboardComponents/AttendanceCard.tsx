...
export default function AttendanceCard({
  ...
  return (
    <div className="p-4 sm:p-5 lg:p-6">
      ...
      <h2 className="text-sm font-semibold text-gray-800 mt-1">Attendance</h2>
      <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mt-1">{total.toLocaleString("fr-DZ")}</p>
      ...
      <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
        <span className="flex items-center gap-1">
          <span className="w-2.5 h-2.5 rounded-full bg-[#2d3a6e] inline-block shrink-0" />Men
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2.5 h-2.5 rounded-full bg-gray-300 inline-block shrink-0" />Women
        </span>
      </div>
    </div>
  );
