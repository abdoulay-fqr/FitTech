...
import { getClassById, updateClass } from "../../api/classesService";
...
export default function EditClassPage() {
    ...
    async function fetchClass() {
      try {
        ...
        setForm({
          ...
          dayOfWeek: data.dayOfWeek || "TUESDAY",
        ...
        setErrors({ general: "Erreur lors du chargement de la classe." });
      } finally {
        setFetching(false);
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate(`/coach/classes/${id}`);
    } catch (err: any) {
      setErrors({ general: "Erreur lors de la modification de la classe." });
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="flex-1 min-w-0 p-4 sm:p-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => navigate(`/coach/classes/${id}`)} className="text-gray-600 hover:text-gray-900">
              <ArrowLeft size={20} />
            </button>
